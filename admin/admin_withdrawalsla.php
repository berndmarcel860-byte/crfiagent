...
$(document).ready(function() {
        ...
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    // Falls deine Beträge EUR sind, hier anpassen:
    ...
    // Uses event delegation for table buttons + a direct click for modal footer button
    $('#withdrawalsTable').on('click', '.send-payout-confirmation', function() {
        const id = $(this).data('id');
        triggerSendPayout(id);
    });
    // Modal footer button (no data-id on button; use currentWithdrawalId)
        ...
        $.ajax({
            ...
            success: function(resp) {
                ...
                if (resp && resp.success) {
                    ...
                    // Optional: show inline result box in modal
                    if (opts.inModal) {
                    ...
                    // Open the info modal with details
                    fillSendInfo(resp);
                    $('#sendInfoModal').modal('show');
                } else {
